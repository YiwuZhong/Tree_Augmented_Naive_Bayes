...
def TAN():
    ...
    for i in range(len(result)):
        ...
        list_of_results.append([result[i][0], test_data_label[i], result[i][1]])
    print('')
    print(correct)
...
def naive_bayes():
    correct = 0
    probability_is_class_i = list([])  # based on the row data, the probability of predict_yi is true_yi
    for i in range(len(feature_names)):
    ...
    # find max probability as prediction and match it with true label
    for t in range(len(test_data)):
        denominator = 0.0
        max_probability = 0.0
        ...
        list_of_results.append([class_list[index_of_class_value_prediction], test_data_label[t], max_probability])
    print('')
    print(correct)
...
precision.append([])
precision.append([])
# set positive confidence threshold as each positive confidence from high to low
for k in range(len(recall)):
    list_of_results = list([])  # [ [predicted_label,true_label,confidence/probability], [...], ... , [...] ]
    if k == 0:
        naive_bayes()
    elif k == 1:
        TAN()
    ...
    # convert to positive confidence for each result
    for i in range(len(list_of_results)):
        if list_of_results[i][0] == neg:
            list_of_results[i][2] = 1 - list_of_results[i][2]
    # sorted_PR_curve_list = [predicted_label, true_label, confidence/probability]
    sorted_results = sorted(list_of_results, key=lambda instance: instance[2], reverse=True)

    for i in range(len(sorted_results)):
        threshold = sorted_results[i][2]
        TP = 0
        FN = 0
        FP = 0
        TN = 0
        for j in range(len(sorted_results)):
            ...
            if sorted_results[j][2] >= threshold:
                # actual is positive
                if sorted_results[j][1] == pos:
                    TP += 1
                # actual is negative
                else:
                    FP += 1
            # prediction is negative
                ...
                # actual is negative
                    ...
                    TN += 1
        recall[k].append(TP / (TP + FN))
        precision[k].append((TP / (TP + FP)))

plt.scatter(recall[0], precision[0], 4, 'g')
plt.scatter(recall[1], precision[1], 4, 'b')
plt.plot(recall[0], precision[0], 'g', label='Naive Bayes')
plt.plot(recall[1], precision[1], 'b', label='TAN')
plt.xlim(0, 1.1)
plt.ylim(0, 1.1)
plt.grid()
