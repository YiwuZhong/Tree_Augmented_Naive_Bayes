...
def TAN():
    ...
    print(correct)
    # store correct
    accuracy[1].append(correct/len(test_data))
...
def naive_bayes():
    ...
    print(correct)
    # store correct
    accuracy[0].append(correct/len(test_data))
...
# all data
cv_data = train_data
cv_data.extend(test_data)
cv_data_label = train_data_label
cv_data_label.extend(test_data_label)

# [0] is naive bayes; [1] is TAN
accuracy = list([])
accuracy.append([])
accuracy.append([])

# cross-validation: 142 samples 10-fold divided into 15, 15, 14, ... ,14
for i in range(10):
    if i == 0:
        # cross-data
        train_data = cv_data[15:142]
        train_data_label = cv_data_label[15:142]
        test_data = cv_data[0:15]  # [a,b)
        test_data_label = cv_data_label[0:15]
        # run classifier algorithm
        naive_bayes()
        TAN()
    elif i == 1:
        # cross-data
        train_data = cv_data[0:15]
        train_data.extend(cv_data[30:142])
        train_data_label = cv_data_label[0:15]
        train_data_label.extend(cv_data_label[30:142])
        test_data = cv_data[15:30]
        test_data_label = cv_data_label[15:30]
        # run classifier algorithm
        naive_bayes()
        TAN()
    elif i == 9:
        # cross-data
        train_data = cv_data[0:128]
        train_data_label = cv_data_label[0:128]
        test_data = cv_data[128:142]
        test_data_label = cv_data_label[128:142]
        # run classifier algorithm
        naive_bayes()
        TAN()
    else:
        # cross-data
        train_data = cv_data[0:30+(i-2)*14]
        train_data.extend(cv_data[30+(i-1)*14:142])
        train_data_label = cv_data_label[0:30+(i-2)*14]
        train_data_label.extend(cv_data_label[30+(i-1)*14:142])
        test_data = cv_data[30+(i-2)*14:30+(i-1)*14]
        test_data_label = cv_data_label[30+(i-2)*14:30+(i-1)*14]
        # run classifier algorithm
        naive_bayes()
        TAN()
...
for j in range(len(accuracy[0])):
    delta.append((accuracy[1][j] - accuracy[0][j]))
    sum_difference += delta[j]

delta_mean = sum_difference/len(accuracy[0])
sum_D = 0.0
# calculate t statistic
for j in range(len(delta)):
    sum_D += (delta[j]-delta_mean) * (delta[j]-delta_mean)
t = delta_mean/(math.sqrt(sum_D/(10*9)))
print(accuracy)
print(delta)
print(delta_mean)
print(t)
